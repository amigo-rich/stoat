...
impl Store {
	...
	let result = self.con.query_row(sql, params![id], |row| {
	    ...
	    Ok(Image{id: row.get(0)?, hash: row.get(1)?, path: Path::new(&path_as_string).to_path_buf()})
	});
	match result {
	    Ok(image) => Ok(image),
    ...
    pub fn put_image_location(&self, name: &str, latitude: f32, longitude: f32) -> Result<Location, Error> {
	let sql = r#"
	...
	"#;
	let result = self.con.query_row(sql, params![id], |row| {
	    Ok(Location{id: row.get(0)?, name: row.get(1)?, latitude: row.get(2)?, longitude: row.get(3)?})
	});
	match result {
